...
## -----------------------------------
    ...
    def setup_initial_conditions(self):
        ...
        # this line has been changed to allow reading MESA data
            ...
            if self.itype==self.HEGER:
                ...
                # TODO EDIT using the example for the HEGER star just above this, add code to read data from the MESA file
                # note that the variable names from m.read_mesa_star() are things like
                # m.data['radius'], m.data['logP'], m.data['logRho']
                # the log variables are log base 10, the radius is in units of RSun, mass is in units of MSun
                # all quantities will need to be converted to cgs units
                # note also that the MESA grid is defined from outside in, so you'll need to reverse the quantities
                # when assigning to hydro code variables
                
                m=mesa.mesa(path=self.mpath,name=self.mname)
                # TODO EDIT add a line that reads the data into the object m
                
                if self.hent > 0:
                    s4, = np.where(m.data['entropy'] > self.hent)

                    # TODO EDIT define the inner and outer radius
                    # again note that the m.data['radius'] variable is in units of RSun and goes from outside in
                    self.r_inner = 
                    self.r_outer = 
                    print('r inner: ',self.r_inner)
